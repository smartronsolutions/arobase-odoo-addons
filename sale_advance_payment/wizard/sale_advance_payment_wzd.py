...
class AccountVoucherWizard(models.TransientModel):
    ...
    amount_advance = fields.Float(string="Amount advanced", required=True,
    ...
    currency_amount = fields.Float(string="Curr. amount", digits=(16, 2),
                                   readonly=True)
    payment_ref = fields.Char(string="Ref.")

    # Utiliser payment_method_line_id avec un display_name personnalisé
    ...
    def make_advance_payment(self):
        ...
        if sale_ids:
            ...
            payment_res = {'payment_type': 'inbound',
                            ...
                            'currency_id': self.journal_id.currency_id.id or company.currency_id.id,
                            'date': date,
                            'amount': self.amount_advance,
                            'sale_id': sale.id,
#                            'name': _("Advance Payment") + " - " + sale.name,
#                            'ref': self.payment_ref or sale.name,
                            'ref': _("Advance Payment") + " - " + sale.name,
