...
class KsWooWebhookHandler(http.Controller):
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/customer/create'], auth='none', csrf=False,
                methods=['POST'])
    def create_customer_webhook(self, db, woo_instance_id, uid, **post):
        if request.env['ks.woo.connector.instance'].sudo().search([('ks_instance_state', 'in', ['active'])]):
            try:
                ...
                request.env['ks.woo.webhook.logger'].ks_create_webhook_log_param(
                    ks_operation_performed="customer_create",
                    ks_status="draft",
                    ks_type="webhook",
                ...
                if data_j:
                    ...
                    if woo_instance_id:
                        if woo_instance and data_j:
                            ...
                            request.env['ks.woo.queue.jobs'].ks_create_customer_record_in_queue(instance=woo_instance,
                                                                                                woo=True,
                                                                                                data=[data_j])
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/customer/update'], auth='none', csrf=False,
                methods=['POST'])
    def update_customer_webhook(self, woo_instance_id, db, uid, **post):
        if request.env['ks.woo.connector.instance'].sudo().search([('ks_instance_state', 'in', ['active'])]):
            try:
                ...
                data_j = json.loads(data_json)
                request.env['ks.woo.webhook.logger'].ks_create_webhook_log_param(
                    ks_operation_performed="customer_update",
                    ks_status="draft",
                    ks_type="webhook",
                    ks_woo_instance=woo_instance,
                    ks_woo_id=data_j['id'],
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/coupon/create'], auth='none', csrf=False,
                methods=['POST'])
                ...
                if data_j:
                    ...
                    if woo_instance_id:
                        if woo_instance and data_j:
                            request.env.company = woo_instance.ks_company_id
                            request.env.companies = woo_instance.ks_company_id
                            request.env['ks.woo.queue.jobs'].ks_create_coupon_record_in_queue(instance=woo_instance,
                                                                                              woo=True,
                                                                                              data=[data_j])
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/coupon/update'], auth='none', csrf=False,
                methods=['POST'])
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/product/create'], auth='none', csrf=False,
                methods=['POST'])
    def create_product_webhook(self, woo_instance_id, db, uid, **post):
        if request.env['ks.woo.connector.instance'].sudo().search([('ks_instance_state', 'in', ['active'])]):
            try:
                ...
                data_j = json.loads(data_json)
                request.env['ks.woo.webhook.logger'].ks_create_webhook_log_param(
                    ks_operation_performed="product_create",
                    ks_status="draft",
                    ks_type="webhook",
                    ks_woo_instance=woo_instance,
                    ks_woo_id=data_j['id'],
                        ...
                        if woo_instance and data_j:
                            ...
                            if woo_instance.ks_woo_auto_stock_import:
                                request.env['ks.woo.queue.jobs'].sudo().ks_import_stock_woocommerce_in_queue(
                                    woo_instance, data_j)
                            request.env['ks.woo.queue.jobs'].ks_create_product_record_in_queue(instance=woo_instance,
                                                                                               woo=True,
                                                                                               data=[data_j])
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/product/update'], auth='none', csrf=False,
                methods=['POST'])
    def update_product_webhook(self, woo_instance_id, db, uid, **post):
        if request.env['ks.woo.connector.instance'].sudo().search([('ks_instance_state', 'in', ['active'])]):
            try:
                ...
                data_j = json.loads(data_json)
                request.env['ks.woo.webhook.logger'].ks_create_webhook_log_param(
                    ks_operation_performed="product_update",
                    ks_status="draft",
                    ks_type="webhook",
                    ks_woo_instance=woo_instance,
                    ks_woo_id=data_j['id'],
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/order/create'], auth='none', csrf=False,
                methods=['POST'])
        ...
        if request.env['ks.woo.connector.instance'].sudo().search([('ks_instance_state', 'in', ['active'])]):
            try:
                ...
                if data:
                    ...
                    data_j = json.loads(data_json)
                    request.env['ks.woo.webhook.logger'].ks_create_webhook_log_param(
                        ks_operation_performed="order_create",
                        ks_status="draft",
                        ks_type="webhook",
                        ks_woo_instance=woo_instance,
                        ks_woo_id=data_j['id'],
                        ...
                        if woo_instance_id:
                            if woo_instance and data_j:
                                ...
                                if data_j.get('status', False) in ks_woo_sync_status:
                                    request.env['ks.woo.queue.jobs'].ks_create_order_record_in_queue(
    ...
    @http.route(['/woo_hook/<string:db>/<string:uid>/<int:woo_instance_id>/order/update'], auth='none', csrf=False,
                methods=['POST'])
